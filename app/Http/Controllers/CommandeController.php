...
class CommandeController extends Controller
{
    public function getCommende()
    {
        $session_id=Session::get('session_id');
               ...
               $cat = categorie::where('parent_id','=', 0)->orderby('nom', 'asc')->get();
        // dd($cat);
    ...
    public function envoyeconfirmationcommande($commande)
    {
      Mail::to($commande->email)->send(new OrderMail($commande));
    ...
    {
        $commande=commande::where('Partenaire', '=', 0)->orderBy('id', 'desc')->paginate(20);
    ...
    {
        $this->envoyeconfirmationcommande($commande);
    ...
    {
        $data=$request->recherche;
        $part=Partenaire::where('id_user', '=', Auth::user()->id)->get();
    ...
    {
        $Partenaire=Partenaire::where('id_user', '=', $id)->get();
        foreach ($Partenaire as $f) {
            $commande=commande::where('Partenaire', '=', $f->id)->orderBy('id', 'desc')->get();
        }
    ...
    {
        $session_id=Session::get('session_id');
        $Partenaire=Partenaire::where('id_user', '=', Auth::user()->id)->get();

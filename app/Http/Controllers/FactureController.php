...
class FactureController extends Controller
{
  //Afficher Facture des client
	public function index(){
		$facture=facture::where('id_partenaires','=',0)->where('id_commande','>',0)->orderBy('id','desc')->paginate(20);
  ...
  public function index2(){
    $facture=facture::where('id_partenaires','>',0)->orderBy('id','desc')->paginate(20);
    return view('dashboard.tous_factures')->with(array(
      'facture'=>$facture,
    ));
  }
    ...
    public function FactureF(){
        ...
        $Partenaire=Partenaire::where('id_user','=',$id_user)->paginate(20);
        foreach ($Partenaire as $f) {
          $facture=facture::where('id_partenaires','=',$f->id)->orderBy('id','desc')->paginate(20);
        }
      ...
      if ($request->isMethod('post')) {
        ...
        $produit_pack=pack_produit::all();
          ...
          if ($countfact > 0) {
            ...
            $fact->tranche=0;
            $fact->rendu=0;
            if ($commande->Partenaire > 0) {
              $fact->id_partenaires=$commande->Partenaire;
              $fact->methode="Comptant";
            }else{
              $fact->id_partenaires=0;
    ...
    public function facture_fournisseur($id){
      ...
      $part=Partenaire::where('id_user','=',$id)->get();
...
/****fonction pour retourner les produit de la facture a la page facture manuel (facture qui est imprimer)*****/
    ...
    public function get_element_update_invoice($id){
      $facture=facture::find($id);
            $part=Partenaire::where('id','=',$facture->id_partenaires)->get();
            $produit_dropdown=produit::with('attributes')->get();
    ...
    public function SearchDate(Request $request){
      $data=$request->recherche;
      $id_user=Auth::user()->id;
      $Partenaire=Partenaire::where('id_user','=',$id_user)->get();
      foreach ($Partenaire as $p) {
          $facture=facture::where('date','like','%'.$data.'%')->where('id_partenaires','=',$p->id)->paginate(20);
      }
      if($facture != NULL){
        return view('dashboard.tous_factures')->with(array(
          'facture'=>$facture,
    ...
    public function SearchInvoicePersonnal(Request $request){
      $data=$request->recherche;
      $facture=facture::where('client','like','%'.$data.'%')->where('id_commande','=',0)->where('id_partenaires','=',0)->get();
      if($facture != NULL){
        return view('dashboard.facturePersonnel')->with(array(
          'facture'=>$facture,
        ));
    ...
    public function DeleteAllSelected(Request $request){
      $data=$request->all();
        foreach ($data['all'] as $tous => $val) {
            ...
            if ($facture->id_commande == 0 && $facture->id_partenaires == 0) {

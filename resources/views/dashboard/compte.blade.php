          ...
          <div class="btn-group">
           <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">Ajouter</button>
            <div class="dropdown-menu">
              ...
              <a class="dropdown-item" title="Télécharger" href="{{ url('/Ajout_Fournisseur')}}">Partenaire</a>
            </div>
          </div>
                    ...
                    <div class="table-responsive">
                      <table class="table">
                        ...
                        <tbody>
                          ...
                          <td> {{ $row->email }} </td>
                          <td>@if($row->role == 2)
                                @foreach($Partenaire as $pr)
                                  @if($pr->id_user == $row->id)
                                   {{ $pr->pass }}
                                  @endif
                                @endforeach
                              @endif 
                          ...
                          <td>
                            @if($row->role == 2)
                              <button type="button" id="ForDelete"class="btn btn-danger btn-rounded btn-icon" data-toggle="modal" data-target="#Delete" data-fourni="{{ $row->id }}" data-type="Partenaire" title="Supprimer"><i class="mdi mdi-delete"></i>
                              </button>
                            @endif
